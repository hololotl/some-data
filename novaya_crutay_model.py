...
def train_catboost(train_df, features, cat_features):
...
def train_linear_baseline(train_df, features, cat_features):
    df = train_df.copy()
    df = pd.get_dummies(df, columns=cat_features, drop_first=True)

    dummy_prefixes = [f"{c}_" for c in cat_features]
    dummy_cols = [
        c for c in df.columns if any(c.startswith(p) for p in dummy_prefixes)
    ]
    baseline_features = features + dummy_cols

    X_train = df[baseline_features].select_dtypes(include=[np.number])
    y_train = df["orders_count"]
...
def evaluate_model(model, test_df, features, cat_features, clip_negative):
    df = test_df.copy()
...
def evaluate_linear_baseline(model, test_df, baseline_features, cat_features):
    df = test_df.copy()
    df = pd.get_dummies(df, columns=cat_features, drop_first=True)

    X = (
        df.reindex(columns=baseline_features, fill_value=0)
    ...
    return {"mae": mae, "rmse": rmse, "rows": len(result_df)}
...
def save_outputs(
    ...
    model_params_df = pd.DataFrame({
        "feature": ["intercept"],
        "coefficient": [model.get_param("model_shrink_rate") if hasattr(model, "get_param") else np.nan],
    })
    ...
    model_params_df = pd.concat(
        [
            ...
            .rename(columns={"importance": "coefficient"})
        ],
        ignore_index=True
    )
...
def print_feature_importance(model, features, cat_features):
...
def main():
    ...
    peak_metrics_df, peak_examples_df = compute_peak_metrics(
        train_df,
        result_df
    )
    ...
    baseline_model, baseline_features = train_linear_baseline(
        train_df,
        features,
        cat_features
    )

    baseline_metrics = evaluate_linear_baseline(
        baseline_model,
        test_df,
        baseline_features,
        cat_features
    ...
    mae_improvement = baseline_metrics["mae"] - global_metrics["mae"]
    rmse_improvement = baseline_metrics["rmse"] - global_metrics["rmse"]
